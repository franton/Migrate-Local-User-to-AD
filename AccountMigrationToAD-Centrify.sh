...
# Script to migrate local accounts to Centrify Active Directory accounts
...
AreWeADBound()
{
	adbound=$( /usr/local/bin/adinfo | grep "Licensed Features" | awk '{ print $3 }' )
	
	if [ "${adbound}" != "Enabled" ];
	then
		/bin/echo "This machine is not bound to Centrify Active Directory."
		adbound="no"
		errorcode=1
	else
		/bin/echo "Bound to Centrify Active Directory. Proceeding."
		adbound="yes"
	fi
}
...
MigrateAccount()
{
	...
	/usr/sbin/chown -R $adusername:staff /Users/$adusername
	...
	# Delete user keychain or we'll have issues.
	KC=$( /usr/bin/su $adusername -c "security list-keychains" | grep login | sed -e 's/\"//g' | sed -e 's/\// /g' | awk '{print $NF}' )
	/usr/bin/su $adusername -c "security delete-keychain $KC"
	...
	/usr/bin/expect <<- DONE
	spawn /usr/bin/fdesetup add -usertoadd $adusername
